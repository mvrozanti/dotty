...
SAFE_NAMES = ['dotty','.git', '.gitignore', 'README', 'LICENSE', 'test'] # maybe this should be part of config?
...
#                 dst = dst.replace(result2, result1)
    ...
    if dry_run:
        dry_run_events.append('would copy: %-60s -> %s' % (src, dst))
        return
    if op.isfile(src):
        if not dry_run: print("Copying %-60s -> %s" % (src, dst))
